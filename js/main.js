...
import { Zombie } from './entities/Zombie.js?v=29';
...
const audioSystem = {
    ...
    // Initialize weapon sound effects
    initWeaponSounds() {
        // Load weapon sounds
        this.soundEffects.pistol = new Audio('assets/audio/weapons/sound_pistol.mp3');
        this.soundEffects.shotgun = new Audio('assets/audio/weapons/sound_shotgun.mp3');
        this.soundEffects.machinegun = new Audio('assets/audio/weapons/sound_machinegun.mp3');
        this.soundEffects.zombieRound = new Audio('assets/audio/weapons/sound_zombieround.mp3');
        
        // Set volume for weapon sounds
        this.soundEffects.pistol.volume = 0.4;
        this.soundEffects.shotgun.volume = 0.5;
        this.soundEffects.machinegun.volume = 0.3;
        this.soundEffects.zombieRound.volume = 0.6;
    },
    
    // Play a weapon sound
    playWeaponSound(weaponType) {
        const sound = this.soundEffects[weaponType];
        if (sound) {
            // Check if the sound is already playing
            if (!sound.paused && sound.currentTime > 0) {
                // If it's playing, reset it to start
                sound.currentTime = 0;
            }
            // Play the sound
            sound.play().catch(error => {
                console.warn(`Weapon sound playback failed: ${weaponType}`, error);
            });
        }
    },
    
    // Play zombie round sound
    playZombieRoundSound() {
        if (this.soundEffects.zombieRound) {
            this.soundEffects.zombieRound.play().catch(error => {
                console.warn('Zombie round sound playback failed:', error);
            });
        }
    },
...
// Define canvas variable to reference the renderer's DOM element
...
const player = {
    ...
    // Add takeDamage method for compatibility
...
function handleShooting() {
    ...
    if (player.mouseDown && !player.isShooting && now - player.lastShootTime > cooldown) {
        ...
        audioSystem.playWeaponSound(player.currentWeapon);
...
function spawnWaveZombies() {
    ...
    if (!waveSystem.isWaveInProgress) {
        // Safety check to ensure we have a valid wave index
        if (waveSystem.currentWave < 1 || waveSystem.currentWave > waveSystem.maxWaves) {
            console.error(`Invalid wave number: ${waveSystem.currentWave}, resetting to 1`);
        ...
        if (waveSystem.currentWave === 5) {
            ...
            // Spawn 30 zombies at these points
            for (let i = 0; i < 30; i++) {
                ...
                while (!validPosition && attempts < 15) {
                    attempts++;
                    // Increased spread from 5 to 15 units for better spacing
                    ...
                    if (validPosition) {
                        for (const existingZombie of zombies) {
                            ...
                            // Increased minimum distance between zombies from 3 to 5 units
                ...
                if (validPosition) {
                    ...
                    zombie.mesh.userData.startDelay = Math.random() * 2000;
                    zombies.push(zombie);
                } else {
                    // Fallback: spawn at a random position around the map perimeter
                    const angle = Math.random() * Math.PI * 2;
                    const distance = 50 + Math.random() * 5;
                    x = player.position.x + Math.cos(angle) * distance;
                    z = player.position.z + Math.sin(angle) * distance;
                    const zombie = new Zombie(scene, x, z, false);
                    zombie.mesh.userData.startDelay = Math.random() * 2000;
            ...
            // For normal waves, spawn zombies based on the wave number
...
function handleGameWin() {
    // Set game over state in both places for consistency
...
function updateZombies() {
    ...
    for (let i = zombies.length - 1; i >= 0; i--) {
        ...
        // Calculate intended position using the zombie's speedMultiplier
        const baseSpeed = ZOMBIE.SPEED;
        const speedMultiplier = zombie.mesh.userData.speedMultiplier || 1.0; // Default to 1.0 if not set
...
function attackPlayer(zombie) {
    ...
    const damaged = player.takeDamage(zombie.mesh.userData.damage);
...
function restartGame() {
    ...
    try {
        ...
        // Remove game over or win display - use a safer approach
        const gameEndDisplay = document.querySelector('.game-menu:not(#startScreen):not(#pauseMenu)');
        if (gameEndDisplay && gameEndDisplay.parentNode) {
            gameEndDisplay.parentNode.removeChild(gameEndDisplay);
        }
        ...
        waveSystem.isWaveInProgress = false;
        waveSystem.isWaveTransition = false;
        waveSystem.zombiesRemaining = 0; // Reset zombies remaining to trigger new wave
        updateWaveIndicator();
        ...
        // Force a new wave to start after a short delay
        setTimeout(() => {
            spawnWaveZombies();
        }, 1000);
...
function updateHealthDisplay() {
    ...
    if (player.health <= 0) {
        ...
        console.log("Health reached zero, triggering game over"); // Debug log
        
        // Update the health bar to show zero health
        const healthPercent = 0;
        healthBar.style.width = `${healthPercent}%`;
        healthBar.style.backgroundColor = 'red';
        
        // Call handleGameOver after a short delay to ensure UI updates first
        setTimeout(() => {
            handleGameOver();
        }, 50);
...
// Define timing variables before they're used
...
function animate() {
    ...
    if (!player.isGameOver && !gameState.isPaused) {
        ...
        // Keep flashlight and player light updated
        flashlight.position.copy(camera.position);
        playerLight.position.copy(camera.position);
...
function handleGameOver() {
    // Check if already in game over state to prevent multiple calls
    if (player.isGameOver) return;
    ...
    try {
        ...
        // Ensure the game over display is visible before adding it to the DOM
        gameOverDisplay.style.display = 'block';
        document.body.appendChild(gameOverDisplay);
        
        // Ensure the game over display is visible
        setTimeout(() => {
            if (gameOverDisplay.parentNode) {
                gameOverDisplay.style.display = 'block';
            }
